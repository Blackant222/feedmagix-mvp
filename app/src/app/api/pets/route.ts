...
export async function GET(request: NextRequest) {
  try {
    ...
    const actualUserId = session.userId;
    
    let userPets;
    try {
      logger.debug('Querying pets table', { userId: actualUserId });
      userPets = await db
        .select()
        .from(pets)
        .where(eq(pets.userId, actualUserId))
        .orderBy(pets.createdAt);
    } catch (dbError) {
      logger.warn('Primary pets query failed, trying simpler query', {
      ...
      try {
        ...
        logger.error('All pets queries failed', {
          primaryError: dbError instanceof Error ? dbError.message : 'Unknown error',
          fallbackError: simpleError instanceof Error ? simpleError.message : 'Unknown error',
          userId: actualUserId
        });
    ...
    logger.debug('Database pets query result', {
    ...
    logger.businessEvent('pets_api_success', {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertData: typeof pets.$inferInsert = {
      name: petData.name,
      species: petData.type, // Map frontend type to database species
      userId: user.user.id,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    ...
    const transformedPet = {
      ...
      imageUrl: newPet.avatarUrl,
      dietaryRestrictions: newPet.dietaryRestrictions || [],
      healthConditions: newPet.healthConditions || [],
