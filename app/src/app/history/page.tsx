...
export default function HistoryPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        logger.debug('Fetching analysis history', {}, user?.id);
        ...
        if (response.error) {
          ...
          if (response.error.code === 'unauthorized' || response.error.code === 'UNKNOWN_ERROR') {
            logger.warn('Auth failed for history fetch, trying direct API call', {}, user?.id);
            try {
              ...
              if (directData.success && directData.data) {
                ...
                logger.debug('Processing analysis history data', {
                ...
                logger.businessEvent('analysis_history_loaded', {
                  historyCount: transformedHistory.length,
                  source: 'direct_api'
                }, user?.id);
                setHistory(transformedHistory);
                return;
              } else {
                logger.warn('No data in direct API response', {}, user?.id);
              }
            } catch (directError) {
              logger.error('Direct API call failed', { error: directError }, user?.id);
            }
          }
        ...
        const analyses = (backendData?.analyses || []) as unknown as HistoryAnalysisItem[];
        logger.debug('Processing main API analysis history data', {
          analysesCount: analyses.length
        }, user?.id);
        ...
        logger.error('Failed to load history data', { error: err }, user?.id);
  ...
  const handleDeleteScan = async (scanId: string) => {
    if (confirm('آیا مطمئن هستید که می‌خواهید این اسکن را حذف کنید؟')) {
      try {
        ...
        logger.businessEvent('analysis_deleted', { analysisId: scanId }, user?.id);
        toast({
          title: 'موفق',
          description: 'اسکن با موفقیت حذف شد',
          variant: 'default'
        });
      } catch (err) {
        logger.error('Failed to delete scan', { error: err, scanId }, user?.id);
        toast({
          title: 'خطا',
          description: 'خطا در حذف اسکن',
          variant: 'destructive'
        });
      }
    }
  ...
  const stats = {
    ...
    averageScore: history.length > 0 ? Math.round(
